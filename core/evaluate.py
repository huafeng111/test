...
def get_composition_loss_whole_img(img, alpha, fg, bg, predict):
    ...
    comp_loss = torch.sqrt((comp - img) ** 2 + 1e-12)  # fixed bug, remove "/255."
    comp_loss = comp_loss.sum()/(weighted.sum())
    return comp_loss

def get_composition_loss(img, alpha, fg, bg, trimap, predict):
...
##############################
def calculate_sad_mse_mad_privacy(predict_old, alpha, privacy):
    predict = np.copy(predict_old)
    pixel = float((privacy == 255).sum())
    sad_diff_mask = np.abs(predict - alpha)
    sad_diff_mask[privacy != 255] = 0.
    sad_diff = np.sum(sad_diff_mask) / 1000
    if pixel == 0:
        return 0, 0, 0
    mse_diff = np.sum(sad_diff_mask ** 2)/pixel
    mad_diff = np.sum(sad_diff_mask)/pixel
